...
def create_api_clients(multi_config):
  ...
  return {
    "sensors": SensorsApi(host=url, key=key, username=user, password=password),
    "streams": StreamsApi(host=url, key=key, username=user, password=password),
    "datapoints": DatapointsApi(host=url, key=key, username=user, password=password)
  }
...
def upload_datapoint(record, multi_config):
  ...
  for sensor_name in sensor_names:
    ...
    for x in parameters:
      if x in record.keys() and parameters[x]["sensor_name"] == sensor_name:
        data_value = record[x]
